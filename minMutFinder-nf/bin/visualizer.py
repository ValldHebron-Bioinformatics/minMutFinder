...
##### ########################################################################
    ...
    if not annotate.empty:
        ...
        # Las columnas ya llegan correctamente nombradas desde annotate_and_viz.py;
        # cualquier columna extra (Antiviral, mAb, Phenotype, Fold*...) se detecta por nombre,
        # no por posición, para no depender del orden en que se hayan añadido
        annot_m = annotate
        extra_annotation_columns = [c for c in annot_m.columns if c not in base_annot_columns]
        ...
        for i in annot_m.Aa_change:
            ...
            for col in extra_annotation_columns:
                muts.loc[muts.Aa_change == i, col] = annot_m.loc[annot_m.Aa_change == i, col].values[0]

...
process vizNoAnnot {
    ...
    script:
    """
    mkdir -p ${out_path}/mutations
    cp -f ${muts} ${out_path}/mutations/${out_path.baseName}_mutations.csv

    python3 ${params.programs.viz_no_annotate} --out-dir ${out_path} --ref-seq ${ref_seq} --prot-names ${out_path}/assembly/prot_names.txt --syn-muts ${params.syn_muts}
...
process annotateAndViz {
    ...
    python3 ${params.programs.annotate_and_viz} --out-dir ${out_path} --annotate ${params.annotate} --ref-seq ${ref_seq} --prot-names ${out_path}/assembly/prot_names.txt --syn-muts ${params.syn_muts}
    rm ${out_path}/assembly/prot_names.txt
    """
}

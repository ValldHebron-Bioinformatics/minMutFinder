...
process inVcf {
    ...
    if [[ ${params.vcf} == *".vcf"* ]]; then
        python3 ${params.programs.in_vcf} --out-dir ${params.out_path} --vcf ${params.vcf}
    else
        python3 ${params.programs.tsv2vcf} --out-dir ${params.out_path} --tsv ${params.vcf}
    fi
...
process inVcf_noSB {
    ...
    echo -e "\nBCFTOOLS\n->Variant Calling Step: Consensus sequence obtantion\n"
    if [[ ${vcf} == *".vcf"* ]]; then
        python3 ${params.programs.in_vcf} --out-dir ${out_path} --vcf ${vcf}
    else
        python3 ${params.programs.tsv2vcf} --out-dir ${out_path} --tsv ${vcf}
    fi
...
process inAlignedReads {
    ...
    script:
    """
    python3 ${params.programs.sam_bam_processing} --out-dir ${out_path} --areads ${areads}
    """
}

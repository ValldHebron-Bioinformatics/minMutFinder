...
def helpMessage() {
    log.info """
    minMutFinder v1.1.1 under GPL-3.0 license
...
workflow MAIN_WORKFLOW {
    ...
    samfile_only = out_vcf.map { item -> item[2] }
    fasta_af_only = out_vcf.map { item -> item[8] }
    vcf_af_only = out_vcf.map { item -> item[5] }   // NUEVO
    names = protNames(params.out_path, out_vcf)
    out_muts = byProteinAnalysis(params.out_path, names.flatten(), params.AF, params.depth, out_depth, samfile_only, ref_only, fasta_af_only, vcf_af_only)
    stopper_only = out_muts.map { item -> item[0] }
    out_wait = waitForOutMuts(params.out_path, stopper_only.last())
